...
SCOREFILE = "/home/bananapi/.sopel/modules/unoscores.txt"
...
THEME_NAMES = dict((v, n) for (n, v) in list(THEMES.items()))
...
class UnoBot:
    ...
    def rankings(self, bot, trigger, toplist=NO):
        ...
        if not scores:
            ...
            return
        order = sorted(list(scores.keys()), key=lambda k: scores[k]['points'], reverse=YES)
        if toplist:
            i = 1
    ...
    def game_ended(self, bot, trigger, winner):
        with lock:
            ...
            try:
                ...
                self.update_scores(bot, list(game.players.keys()), winner, score,
    ...
    def set_card_theme(bot, trigger):
        ...
        if theme not in THEMES:
            bot.say(STRINGS['THEME_NEEDED'] % ', '.join(list(THEMES.keys())))
            return
...
def unogames(bot, trigger):
    ...
    pending = 0
    with lock:
        for chan in list(unobot.games.keys()):
